...
{
    private $xml = null;
    private $base_url = 'http://pdf.ipvp.cn/';        //域名
    private $wechat_appid = 'wx7b4fd24bc48ede17';        //微信公众号唯一标识
    private $wechat_appsecret = '92c9f596b3e19fae5363f19c4951285d';  //微信公众号appsecret
    private $alipay_appid = '';  //支付宝服务窗 app_id
    ...
    public function login(){
        return json(['isSucceed'=>true,'data'=>['token'=>'2323dsdf','userName'=>'wwww']]);
    }
    ...
    {
        //1.如果是支付宝，进支付宝支付页
        //2.如果是微信，先跳转到授权页面，回调地址获取授权code ，请求换取access_token,获取openid
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
            ...
            //重定向微信授权
            $this->redirect("https://open.weixin.qq.com/connect/oauth2/authorize?appid=" . $this->wechat_appid . "&redirect_uri=" . urlencode($this->base_url . "index.php/index/Index/wechatPayPage?store_id=" . $store_id) . "&response_type=code&scope=snsapi_base&state=123#wechat_redirect");
        }
        //判断是不是支付宝
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'AlipayClient') !== false) {

            return $this->fetch('index', ['trade_type' => 'FWC', 'store_id' => $store_id, 'openid' => '', 'url' => $this->base_url . "index.php/index/Index/order"]);
            //$this->redirect("https://openauth.alipay.com/oauth2/publicAppAuthorize.htm?app_id=".$this->alipay_appid."&scope=auth_base&redirect_uri=" . urlencode($this->base_url."index.php/index/Index/aliPayPage?store_id=" . $store_id) );
        }
    ...
    public function wechatPayPage()
    {
        ...
        if (array_key_exists("access_token", $arr)) {
            ...
            return $this->fetch('index', ['trade_type' => 'JSAPI', 'store_id' => $store_id, 'openid' => $arr['openid'], 'url' => $this->base_url . "index.php/index/Index/order"]);
            //模拟提交订单
            //$this->redirect($this->base_url.'index.php/index/index/order?openid='.$arr['openid']);
    ...
    public function order()
    {
        ...
        if (empty($order_amt) || empty($trade_type) || empty($store_id)) {
            return json(['data' => array(), 'code' => 300, 'msg' => '参数错误']);
        }

        if ($trade_type == 'JSAPI') {
            if (empty($openid)) return json(['data' => array(), 'code' => 300, 'msg' => '用户授权信息获取失败']);
        } else {
            $openid = "";
        }
        ...
        //    return json(['data'=>array(),'code'=>300,'msg'=>'订单号生成失败']);
        //}
        ...
        $data['notify_url'] = $this->base_url . "index.php/index/index/payCallback";                  //富友异步通知回调地址 (必须为直接可访问的url，不能携带参数)  256
        $data['trade_type'] = $trade_type;                  //支付方式 JSAPI--公众号支付、FWC--支付宝服务窗   16
        $data['sub_openid'] = $openid;                  //子商户用户标识，（JSAPI时必传）   128
        $data['sub_appid'] = ($trade_type == 'JSAPI') ? $this->wechat_appid : $this->alipay_appid;                  //子商户公众号id   32
        //选填
        $data['curr_type'] = "CNY";
    ...
    public function testPayPage()
    {
        ...
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'MicroMessenger') !== false) {
            ...
            return $this->fetch('wechat_paypage', ['trade_type' => 'JSAPI', 'store_id' => $store_id, 'openid' => 'ovu1GwTV5-c15-PhS4duwOrVKX8M', 'url' => $this->base_url . "index.php/index/Index/order"]);
        }
        //判断是不是支付宝
        if (strpos($_SERVER['HTTP_USER_AGENT'], 'AlipayClient') !== false) {
            return $this->fetch('alipay', ['trade_type' => 'FWC', 'store_id' => $store_id, 'openid' => '', 'url' => $this->base_url . "index.php/index/Index/order"]);
        }
        //return $this->fetch('error', ['msg' => '请使用微信或支付宝客户端扫码支付']);
        return $this->fetch('wechat_paypage', ['trade_type' => 'JSAPI', 'store_id' => $store_id, 'openid' => 'ovu1GwTV5-c15-PhS4duwOrVKX8M', 'url' => $this->base_url . "index.php/index/Index/order"]);
    }
    ...
    public function order_test()
    {
        ...
        $data['ins_cd'] = '08A9999999';               //机构号,接入机构在富友的唯一代码  20
        $data['mchnt_cd'] = '0002900F0370542';                  //商户号, 富友分配给二级商户的商户号  15
        $data['term_id'] = 'berj3wcn';                  //终端号(随机8字节数字字母组合)   8
        $data['random_str'] = '8i08jgeabrkn06jryz02pdcoijq5rqgi';                  //随机字符串   32
        $data['goods_des'] = '商品描述';                  //商品描述    128
        $data['mchnt_order_no'] = 'ipvp2017061314323349485455';                  //商户订单号, 商户系统内部的订单号  30
        $data['order_amt'] = 100;                  //总金额, 订单总金额，单位为分   16
        $data['term_ip'] = '127.0.0.1';                  //终端IP    16
        $data['txn_begin_ts'] = '20170613143233';                  //交易起始时间, 订单生成时间，格式为yyyyMMddHHmmss  14
        $data['notify_url'] = $this->base_url . "index.php/index/index/payCallback";                  //富友异步通知回调地址 (必须为直接可访问的url，不能携带参数)  256
        $data['trade_type'] = 'JSAPI';                  //支付方式 JSAPI--公众号支付、FWC--支付宝服务窗   16
        $data['sub_openid'] = 'ovu1GwTV5-c15-PhS4duwOrVKX8M';                  //子商户用户标识，（JSAPI时必传）   128
        $data['sub_appid'] = 'wxa58576997b358e5f';                  //子商户公众号id   32
        //选填
        $data['curr_type'] = "CNY";
        $data['goods_detail'] = "goods_detail";
        $data['goods_tag'] = "goods_tag";
